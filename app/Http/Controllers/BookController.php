...
class BookController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $books = Book::when(
            $title,
            function ($query, $titleInput) use ($title) {
                return $query->title($titleInput);
            }
        );
            ...
            function () use ($id) {
                return Book::with([
                    'reviews' => function ($query) {
                        return $query->latest(); // Apply latest() to the reviews query
                    }
                ])->withAvgRating()->withReviewsCount()->findOrFail($id);
            }
        );

        return view('books.show', compact('book')); // Ensure the view is returned
    }
